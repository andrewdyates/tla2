...
fn count_case(expr: &Expr) -> usize {
    match expr {
        Expr::Case(arms, other) => {
            let arm_count: usize = arms.iter().map(|arm| count_case(&arm.guard.node) + count_case(&arm.body.node)).sum();
        ...
        | Expr::Implies(a, b) | Expr::Subseteq(a, b) => count_case(&a.node) + count_case(&b.node),
        ...
        Expr::SetEnum(elems) | Expr::Times(elems) => elems.iter().map(|e| count_case(&e.node)).sum(),
        Expr::Apply(f, args) => count_case(&f.node) + args.iter().map(|a| count_case(&a.node)).sum::<usize>(),
