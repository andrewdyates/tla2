...
use crate::error::JitRuntimeError;
use tla_value::Value;
    ...
    pub fn from_initial_state(values: &[Value]) -> Self {
        ...
        for value in values {
            let encoding = match infer_var_layout(value) {
                ...
                // `VarLayout` is `#[non_exhaustive]` because it lives in the
                // `tla-jit-abi` leaf crate. Treat any future variant as a
                // compound value requiring full serialization (conservative
                // fallback — correctness over speed).
                _ => {
                    all_scalar = false;
                    VarEncoding::Compound
                }
    ...
    pub fn from_state_layout(layout: &StateLayout) -> Self {
        ...
        for var_layout in layout.iter() {
            let encoding = match var_layout {
                ...
                // `VarLayout` is `#[non_exhaustive]` (defined in `tla-jit-abi`).
                // Conservative fallback: unknown variants require full compound
                // serialization.
                _ => {
                    all_scalar = false;
                    VarEncoding::Compound
...
impl FlatState {
    ...
    pub fn fingerprint(&self) -> u64 {
        crate::fingerprint::jit_xxh3_fingerprint_64(self.buf.as_ptr(), self.buf.len() as u32)
    }
}
...
mod tests {
    ...
    fn test_flat_state_fingerprint_matches_direct_xxh3() {
        ...
        let fp_via_direct = crate::fingerprint::jit_xxh3_fingerprint_64(
