...
pub(crate) fn cmd_audit(dir: &Path, format: AuditOutputFormat) -> Result<()> {
    ...
    let stem_set = |files: &[PathBuf]| -> HashSet<String> {
        files.iter().filter_map(|p| p.file_stem().map(|s| s.to_string_lossy().into_owned())).collect()
    };
    let tla_set = stem_set(&tla_files);
    let cfg_set = stem_set(&cfg_files);

    let parsed: Vec<ParsedFile> = tla_files.iter().map(|path| {
    ...
    let metrics: Vec<FileMetrics> = parsed.iter()
...
fn try_parse_module(path: &Path, source: &str) -> (Option<Module>, Vec<String>) {
    ...
    let hint = path.file_stem().and_then(|s| s.to_str()).filter(|s| !s.is_empty());
    let lower_result = lower_main_module(FileId(0), &tree, hint);
    if !lower_result.errors.is_empty() {
        let errors = lower_result.errors.iter()
...
fn check_spec_config_pairing(tla_set: &HashSet<String>, cfg_set: &HashSet<String>) -> CheckResult {
    ...
    CheckResult { code: "A001", name: "Spec/config pairing", status, details }
}
...
fn check_parse_validation(parsed: &[ParsedFile]) -> CheckResult {
    ...
    for (path, _, module, errors) in parsed {
        if module.is_none() {
            details.push(format!("{}: {} parse/lower error(s)", path.display(), errors.len()));
            for err in errors.iter().take(3) {
                details.push(format!("  {err}"));
            }
        }
    }
    let status = if details.is_empty() { CheckStatus::Pass } else { CheckStatus::Fail };
    CheckResult { code: "A002", name: "Parse validation", status, details }
}
    ...
    for (path, _, module, _) in parsed {
        ...
        if !expected.is_empty() && module.name.node != expected {
            details.push(format!(
                ...
                path.display(), module.name.node
            ));
        }
    }
    let status = if details.is_empty() { CheckStatus::Pass } else { CheckStatus::Warn };
    CheckResult { code: "A003", name: "Naming conventions", status, details }
}
...
fn check_complexity(parsed: &[ParsedFile]) -> CheckResult {
    ...
    for (path, _, module, _) in parsed {
        ...
        if m.max_nesting_depth > 6 {
            details.push(format!("{}: max nesting depth {} (threshold 6)", path.display(), m.max_nesting_depth));
        }
        if m.max_quantifier_depth > 3 {
            details.push(format!("{}: max quantifier depth {} (threshold 3)", path.display(), m.max_quantifier_depth));
        }
        if m.operator_count > 50 {
            details.push(format!("{}: {} operators (consider splitting)", path.display(), m.operator_count));
        }
    }
    let status = if details.is_empty() { CheckStatus::Pass } else { CheckStatus::Warn };
    CheckResult { code: "A004", name: "Complexity metrics", status, details }
}
...
fn check_anti_patterns(parsed: &[ParsedFile]) -> CheckResult {
    ...
    for (path, _, module, _) in parsed {
        ...
        for unit in &module.units {
            if let Unit::Operator(op) = &unit.node {
                find_unbounded_quantifiers(&op.body.node, &op.name.node, &fname.to_string(), &mut details);
            }
        }
    }
    let status = if details.is_empty() { CheckStatus::Pass } else { CheckStatus::Warn };
    CheckResult { code: "A005", name: "Anti-patterns", status, details }
}
...
fn find_unbounded_quantifiers(expr: &Expr, op_name: &str, fname: &str, out: &mut Vec<String>) {
    match expr {
        Expr::Forall(bounds, body) | Expr::Exists(bounds, body) => {
            for bv in bounds {
                if bv.domain.is_none() {
                    out.push(format!("{fname}: unbounded quantifier `{}` in `{op_name}`", bv.name.node));
                }
                if let Some(d) = &bv.domain { find_unbounded_quantifiers(&d.node, op_name, fname, out); }
            }
            find_unbounded_quantifiers(&body.node, op_name, fname, out);
        }
        Expr::Choose(bv, body) => {
            if bv.domain.is_none() {
                out.push(format!("{fname}: unbounded CHOOSE `{}` in `{op_name}`", bv.name.node));
            }
            if let Some(d) = &bv.domain { find_unbounded_quantifiers(&d.node, op_name, fname, out); }
            find_unbounded_quantifiers(&body.node, op_name, fname, out);
        }
        Expr::If(a, b, c) => { for e in [a, b, c] { find_unbounded_quantifiers(&e.node, op_name, fname, out); } }
        Expr::And(a, b) | Expr::Or(a, b) | Expr::Implies(a, b)
        | Expr::In(a, b) | Expr::Eq(a, b) | Expr::Neq(a, b)
        ...
        | Expr::Powerset(e) | Expr::Domain(e) | Expr::BigUnion(e) => {
            find_unbounded_quantifiers(&e.node, op_name, fname, out);
        }
        Expr::Apply(func, args) => {
            find_unbounded_quantifiers(&func.node, op_name, fname, out);
            for arg in args { find_unbounded_quantifiers(&arg.node, op_name, fname, out); }
        }
        Expr::Let(defs, body) => {
            for def in defs { find_unbounded_quantifiers(&def.body.node, op_name, fname, out); }
            find_unbounded_quantifiers(&body.node, op_name, fname, out);
        }
        Expr::SetEnum(es) | Expr::Tuple(es) => {
            for e in es { find_unbounded_quantifiers(&e.node, op_name, fname, out); }
        }
        ...
        Expr::Case(arms, other) => {
            ...
            if let Some(o) = other { find_unbounded_quantifiers(&o.node, op_name, fname, out); }
        }
        Expr::Label(label) => { find_unbounded_quantifiers(&label.body.node, op_name, fname, out); }
...
fn check_config_completeness(dir: &Path, parsed: &[ParsedFile], cfg_files: &[PathBuf]) -> CheckResult {
    let mut details = Vec::new();
    for (path, _, module, _) in parsed {
        let Some(module) = module else { continue };
        let stem = path.file_stem().and_then(|s| s.to_str()).unwrap_or("");
        let cfg_path = dir.join(format!("{stem}.cfg"));
        if !cfg_files.contains(&cfg_path) { continue; }

        let spec_constants: HashSet<String> = module.units.iter().flat_map(|u| match &u.node {
            Unit::Constant(decls) => decls.iter().map(|d| d.name.node.clone()).collect::<Vec<_>>(),
            _ => Vec::new(),
        }).collect();
        if spec_constants.is_empty() { continue; }

        let cfg_source = match std::fs::read_to_string(&cfg_path) { Ok(s) => s, Err(_) => continue };
        let cfg_constants: HashSet<String> = match tla_check::Config::parse(&cfg_source) {
            Ok(c) => c.constants.keys().cloned().collect(),
            Err(_) => { details.push(format!("{}: .cfg parse error", cfg_path.display())); continue; }
        };
        for name in spec_constants.iter().filter(|c| !cfg_constants.contains(c.as_str())) {
            details.push(format!("{stem}.cfg: CONSTANT `{name}` not assigned"));
        }
    }
    let status = if details.is_empty() { CheckStatus::Pass } else { CheckStatus::Fail };
    CheckResult { code: "A006", name: "Config completeness", status, details }
}
...
fn collect_file_metrics(path: &Path, module: &Module) -> FileMetrics {
    let mut m = FileMetrics { file: path.display().to_string(), ..Default::default() };
    for unit in &module.units {
        match &unit.node {
...
fn expr_depths(expr: &Expr, nest: usize, quant: usize) -> (usize, usize) {
    match expr {
        Expr::Forall(bounds, body) | Expr::Exists(bounds, body) => {
            ...
            for bv in bounds { if let Some(d) = &bv.domain { let (n, q) = expr_depths(&d.node, bn, bq); bn = bn.max(n); bq = bq.max(q); } }
        ...
        Expr::If(a, b, c) => {
            ...
            for e in [a, b, c] { let (n, q) = expr_depths(&e.node, nn, quant); bn = bn.max(n); bq = bq.max(q); }
            (bn, bq)
        }
        Expr::Choose(bv, body) => {
            let (nn, nq) = (nest + 1, quant + 1);
            let (mut bn, mut bq) = (nn, nq);
            if let Some(d) = &bv.domain { let (n, q) = expr_depths(&d.node, nn, nq); bn = bn.max(n); bq = bq.max(q); }
        ...
        Expr::Let(defs, body) => {
            let nn = nest + 1;
            let (mut bn, mut bq) = (nn, quant);
            for def in defs { let (n, q) = expr_depths(&def.body.node, nn, quant); bn = bn.max(n); bq = bq.max(q); }
            let (n, q) = expr_depths(&body.node, nn, quant);
            (bn.max(n), bq.max(q))
        }
        Expr::And(a, b) | Expr::Or(a, b) | Expr::Implies(a, b) | Expr::In(a, b) | Expr::Eq(a, b) => {
            let (n1, q1) = expr_depths(&a.node, nest, quant);
            let (n2, q2) = expr_depths(&b.node, nest, quant);
            (n1.max(n2), q1.max(q2))
        }
        Expr::Not(e) | Expr::Prime(e) | Expr::Neg(e) | Expr::Unchanged(e) => expr_depths(&e.node, nest, quant),
        Expr::Apply(func, args) => {
            let (mut bn, mut bq) = expr_depths(&func.node, nest, quant);
            for arg in args { let (n, q) = expr_depths(&arg.node, nest, quant); bn = bn.max(n); bq = bq.max(q); }
            (bn, bq)
        }
        _ => (nest, quant),
    }
...
fn print_human(dir: &Path, tla: &[PathBuf], cfg: &[PathBuf], checks: &[CheckResult], metrics: &[FileMetrics]) {
    println!("Audit Report: {} ({} .tla, {} .cfg)", dir.display(), tla.len(), cfg.len());
    println!("{}", "=".repeat(70));
    println!();
    println!("  {:<8} {:<26} {:<6} {}", "Code", "Check", "Status", "Details");
    println!("  {}", "-".repeat(64));
    ...
    for c in checks {
        ...
        let detail = if c.details.is_empty() { "OK".into() } else { format!("{} finding(s)", c.details.len()) };
        println!("  {:<8} {:<26} {:<15} {}", c.code, c.name, color, detail);
    }

    let non_passing: Vec<_> = checks.iter().filter(|c| !matches!(c.status, CheckStatus::Pass)).collect();
    if !non_passing.is_empty() {
        println!("\nDetails:\n{}", "-".repeat(70));
        for c in non_passing {
            println!("\n  [{}] {} ({})", c.code, c.name, c.status);
            for d in &c.details { println!("    - {d}"); }
        }
    }

    if !metrics.is_empty() {
        println!("\nComplexity Metrics:\n{}", "-".repeat(70));
        println!("  {:<30} {:>5} {:>5} {:>5} {:>5} {:>5}", "File", "Ops", "Vars", "Cnst", "Nest", "Quant");
        for m in metrics {
            let short = Path::new(&m.file).file_name().map(|f| f.to_string_lossy().into_owned()).unwrap_or_default();
            println!("  {:<30} {:>5} {:>5} {:>5} {:>5} {:>5}", short, m.operator_count, m.variable_count, m.constant_count, m.max_nesting_depth, m.max_quantifier_depth);
        }
    }
    ...
    if w > 0 { print!(", {w} warning(s)"); }
    if f > 0 { print!(", {f} failure(s)"); }
    println!();
}

fn count_statuses(checks: &[CheckResult]) -> (usize, usize, usize) {
    let p = checks.iter().filter(|c| matches!(c.status, CheckStatus::Pass)).count();
    let w = checks.iter().filter(|c| matches!(c.status, CheckStatus::Warn)).count();
    let f = checks.iter().filter(|c| matches!(c.status, CheckStatus::Fail)).count();
    (p, w, f)
}
...
fn print_json(dir: &Path, tla: &[PathBuf], cfg: &[PathBuf], checks: &[CheckResult], metrics: &[FileMetrics]) -> Result<()> {
    let (p, w, f) = count_statuses(checks);
...
mod tests {
    ...
    fn test_parse_validation_error() {
        ...
        let r = check_parse_validation(&[(PathBuf::from("Bad.tla"), "NOT VALID TLA+".into(), m, e)]);
        assert_eq!(r.status, CheckStatus::Fail);
    }
    ...
    fn test_depth_nested_quantifiers() {
        ...
        let fm = collect_file_metrics(Path::new("T.tla"), &m);
        assert!(fm.max_nesting_depth >= 2, "nesting >= 2, got {}", fm.max_nesting_depth);
        assert!(fm.max_quantifier_depth >= 2, "quant >= 2, got {}", fm.max_quantifier_depth);
    }
