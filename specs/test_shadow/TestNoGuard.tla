---------------------------- MODULE TestNoGuard ----------------------------
(* Control test: identical to TestNoLet but with the Forall guard removed.
   If state count differs from TestNoLet, the guard IS filtering. *)
EXTENDS Integers

CONSTANTS Value, Actor, Quorum, Ballot
...
VoteFor(self, b, v) ==
  /\ maxBal[self] \leq b
  /\ DidNotVoteIn(self, b)
  \* Forall guard REMOVED - replaced with TRUE
  /\ TRUE
  /\ TRUE
