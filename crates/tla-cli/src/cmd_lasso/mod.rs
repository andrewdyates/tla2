            ...
            match &lasso {
                ...
                None => {
                    ...
                    if is_success {
                        ...
                        println!(
                            "  Violation found but no lasso structure detected."
                        );
                        println!(
                            "  (Safety violations produce finite traces, not lassos.)"
                        );
                    }
                }
            }
...
fn extract_lasso(check_result: &CheckResult) -> Option<LassoInfo> {
    match check_result {
        CheckResult::LivenessViolation {
            ...
            let stem_actions: Vec<String> = prefix
                .action_labels
                .iter()
                .map(|l| l.to_string())
                .collect();
            let loop_actions: Vec<String> = cycle
                .action_labels
                .iter()
                .map(|l| l.to_string())
                .collect();
