...
impl FlatState {
    ...
    pub fn from_buf(buf: Vec<i64>, var_count: usize) -> Result<Self, JitError> {
    ...
    pub fn fingerprint(&self) -> u64 {
        crate::bfs_step::jit_xxh3_fingerprint_64(self.buf.as_ptr(), self.buf.len() as u32)
    }
}
...
fn value_to_i64(value: &Value) -> Result<i64, JitError> {
    match value {
...
mod tests {
    ...
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_flat_state_fingerprint_matches_bfs_step() {
        let values = vec![
            Value::SmallInt(7),
            Value::SmallInt(11),
            Value::SmallInt(42),
        ...
        let fp_via_bfs_step = crate::bfs_step::jit_xxh3_fingerprint_64(
            flat.as_slice().as_ptr(),
            flat.total_slots() as u32,
        );
        assert_eq!(
            fp_via_flat_state, fp_via_bfs_step,
    ...
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_flat_state_fingerprint_matches_compiled_bfs() {
        let values = vec![Value::SmallInt(7), Value::SmallInt(11)];
        let schema = FlatStateSchema::from_initial_state(&values);
        let flat = state_to_flat(&values, &schema).expect("state_to_flat");

        let fp_flat = flat.fingerprint();
        let fp_compiled = crate::compiled_bfs::state_fingerprint(flat.as_slice());
        assert_eq!(
            fp_flat, fp_compiled,
            "FlatState fingerprint must match compiled_bfs::state_fingerprint"
        );
    }
