...
//
// Extended function utilities: term classification, argument resolution
// explanations, integer function bounds, and relation helpers.

use num_bigint::BigInt;
use z4_core::{Sort, TermData, TermId, TermStore, TheoryLit};

use crate::state::SolverState;

use super::super::{CoreSolver, IntRelation};
...
impl CoreSolver {
    ...
    pub(in crate::core) fn atom_and_polarity(terms: &TermStore, lit: TheoryLit) -> (TermId, bool) {
    ...
    pub(super) fn try_reduce_int_term(terms: &TermStore, state: &SolverState, t: TermId) -> Option<BigInt> {
        let TermData::App(sym, args) = terms.get(t) else {
            return None;
        };
        match sym.name() {
    ...
    /// range (#6353).
    pub(super) fn is_in_valid_range(terms: &TermStore, state: &SolverState, t: TermId, val: &BigInt) -> bool {
